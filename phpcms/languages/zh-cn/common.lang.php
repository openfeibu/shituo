<?php

/*Language Format:
Add a new file(.lang.php) with your module name at /phpcms/languages/
translation save at the array:$LANG
*/


$LANG['index'] = '首页';
$LANG['Chinese'] = '简体中文';
$LANG['English'] = '英文';
$LANG['Arab'] = '阿拉伯语';
$LANG['login'] = '登录账号';
$LANG['line'] = '物流专线' ;
$LANG['current_position'] = '当前位置: ';
$LANG['comeform'] = '来源';
$LANG['hot_news'] = '热点新闻';
$LANG['load_more'] = '点击加载更多';
$LANG['no_more'] = '没有更多了';
$LANG['email'] = '简历发送至';
$LANG['address'] = '地点';
$LANG['responsibility'] = '岗位职责';
$LANG['job_requirements'] = '职位要求';
$LANG['stock_code'] = '入仓代码';
$LANG['enter_stock_code'] = '请输入入仓代码';
$LANG['password'] = '密码';
$LANG['enter_password'] = '请输入密码';
$LANG['login2'] = '登录';
$LANG['no_stock_code'] = '没有入仓代码？请联系客服';
$LANG['leave_name'] = '您的称呼';
$LANG['leave_phone'] = '联系方式';
$LANG['leave_content'] = '留言信息';
$LANG['leave_word'] = '留言';
$LANG['name_not_empty'] = '名字不能为空';
$LANG['phone_not_empty'] = '联系方式不能为空';
$LANG['content_not_empty'] = '留言内容不能为空';
$LANG['success'] = '操作成功';
$LANG['company'] = '公司名称';
$LANG['number'] = '运单单号';
$LANG['query'] = '查询';
$LANG['contact_us'] = '客服电话';
$LANG['more'] = '更多';
$LANG['index_new'] = '新闻动态';
$LANG['newest'] = '最新' ;
$LANG['information'] = '账号信息';
$LANG['account'] = '账号';
$LANG['nickname'] = '昵称';
$LANG['modify_password_suggest'] = '建议您经常修改密码，以保证帐号更加安全。';
$LANG['modify'] = '修改';
$LANG['my_express'] = '我的快件';
$LANG['express_number'] = '运单号';
$LANG['no_express'] = '您还没有快件记录';
$LANG['phone'] = '电话';
$LANG['no_active'] = '该单号暂时查不到动态';
$LANG['not_querying'] = '请输入单号进行查询';
$LANG['exit'] = '注销';
?>
