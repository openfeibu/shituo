...
class index {
	...
	//货物跟踪
	public function cargo_tracking()
	{
		$_userid = $this->_userid;
		$_username = $this->_username;
		$_groupid = $this->_groupid;
		...
		if($_POST['dosubmit'])
		{
			...
			if($waybill_data)
			{
				$db = pc_base::load_model("logistics_model");
				$logistics_data = $db->select("waybill_id = '".$waybill_data['waybill_id']."'",'*','','logistics_id DESC');
			}
		}
        include template('content','cargo_tracking',$default_style);
	}
	//个人中心
	public function member_center()
	{
		...
		$memberinfo = $foreground->memberinfo;
		$_userid = $this->_userid;
		$_username = $this->_username;
		$_groupid = $this->_groupid;
		$siteids = getcache('category_content','commons');
		...
		$db = pc_base::load_model("waybill_model");
		$number = $_POST['search_num'];
		$waybill_data = $db->select("username = '".$memberinfo['username']."'",'*','','waybill_id DESC');
		$db = pc_base::load_model("logistics_model");
		foreach ($waybill_data as $key => $value) {
			$waybill_data[$key]['logistics_data'] = $logistics_data = $db->get_one("waybill_id = '".$value['waybill_id']."'");
		}
		include template('content','member_center',$default_style);
	}
	//JSON 输出
	...
	public function ajaxlists()
	{
        ...
        $default_style = $sitelist[$siteid]['default_style'];
        $CATEGORYS = getcache('category_content_'.$siteid,'commons');
        include template('content','list_ajax_content',$default_style);
	}
	/**
	 * 检查支付状态
	 */
