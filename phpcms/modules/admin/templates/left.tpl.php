...
foreach($datas as $_value) {
	...
	foreach($sub_array as $_key=>$_m) {
		...
		echo '<li id="_MP'.$_m['id'].'" '.$classname.'><a href="javascript:_MP('.$_m['id'].',\'?m='.$_m['m'].'&c='.$_m['c'].'&a='.$_m['a'].$data.'\');" hidefocus="true" style="outline:none;">'.L($_m['name']).'<i class="fa fa-angle-left right"></i></a></li>';
	}
	echo '</ul>';
}
?>
